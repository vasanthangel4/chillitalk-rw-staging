...
class Setting extends MY_Controller{
	...
	public function index() {
		...
		// Country list
		$uri_country = 'http://sws.vectone.com/api/CTPCountry?langCode=ENG';
		  
		$this->rest->format('application/json');
	  
		$result_country = $this->rest->get($uri_country);
	...
	public function change_location(){
		...
		if($valid->run()) {
			...
			if($result_register_rotw->ErrCode == 0) {
				echo '
				<script type="text/javascript">
					alert("Succesfully update your location");
					window.location.href = "'.base_url().$this->session->userdata('lang').'/myaccount/setting/";
				</script>
				';
			}else{
				echo '
				<script type="text/javascript">
					alert("Unsuccesfully update your location");
					window.location.href = "'.base_url().$this->session->userdata('lang').'/myaccount/setting/";
				</script>
				';
			}
